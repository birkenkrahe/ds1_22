...
* Week 10: Data frames, lists and 3rd sprint review
#+attr_html: :width 500px
[[../img/frame.jpg]]

- [ ] Review: data frames (DataCamp)
- [ ] Lecture/practice: data frames
- [ ] 3rd sprint review coming up: first results!
* Review: data frames [DataCamp]
#+attr_html: :width 600px
[[../img/df.png]]

*Create a practice file ~df.org~ with the necessary headers in Emacs.*

1) What is a data frame (technically)?
   #+begin_quote
...
2) How can you create a data frame of two vectors with values ~1 2 3 4
...
3) You've bought a new car. The car company sends you a survey. What
...
4) How would you create such a data frame for a survey?
   #+begin_quote
...
5) Which R commands are used to explore data frames?
   #+begin_src R
...
6) What about ~NA~?
    #+begin_src R
      s_na <- data.frame(survey,NA) # add NA column to data frame
      str(s_na)
    #+end_src
    #+begin_src R
      summary(s_na)
    ...
    #+begin_example
     ...
     Customer           Price     Happiness   NA.         
     Mode:logical   Min.   :1   unhappy:0   Mode:logical  
     TRUE:1         1st Qu.:1   neutral:0   NA's:1        
                    Median :1   happy  :1                 
                    Mean   :1                             
                    3rd Qu.:1                             
                    Max.   :1
...
7) How do you get values from a data frame? For example:
   - the first row
   - the third column
...
8) How can you add another row to the data frame?
   #+begin_src R
     new_survey <- survey # always store intermediate results
     new_survey[2,] <- c("Birkenkrahe",
                         "Kia",
                         "Rio",
                         "2023",
                         FALSE,
...
9) How can you name the observations or rows?
   #+begin_src R
     names(new_survey)
     rownames(new_survey)
     rownames(new_survey) <- c("Car_1", "Car_2")
     new_survey

     new_survey["Car_2",]
   #+end_src

10) How can you subset observations? E.g. for car types from 2023?
    #+begin_src R
      subset(new_survey, Year==2023)
    #+end_src
...
11) How can you extract the ~Make~ from that subset?
    #+begin_src R
      subset(new_survey, Year==2023)[,"Make"]
    #+end_src
   
* Week 11-15: Import/basic plotting/ggplot2/projects
* References
#+attr_html: :width 500px
[[../img/books.jpg]]
