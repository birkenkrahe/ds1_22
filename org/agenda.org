...
* TODO Upload Vectors in R in-class practice file

- I have cleaned up the Canvas course dashboard
- Upload the in-class practice file to Canvas or send me an email

* TODO Review test 5 - vectors and matrices in R
...
2) Which commands creates this matrix?
   ...
   #+begin_src R
     ...
     rbind(c(1,2),c("a","b"))
     cbind(c(1,"a"),c(2,"b"))  # not recommended - implies coercion
   #+end_src
...
6) How do you extract elements from ~qux <- c(10,100,1000)~ that are
   ...
   #+begin_src R
     ...
     which(qux>=100)  # index vector
     (qux >= 100)     # logical vector
...
* TODO Review test 6 - creating, sorting, measuring
...
* TODO Review naming vectors and indexing
...
* TODO (FRIDAY) Lab session: subsetting with ~Nile~
* TODO (FRIDAY) What did you learn so far?
