...
* TODO WED Review test 5 - vectors and matrices in R

1) [ ] Which commands creates this matrix?
   #+begin_example R
          [,1] [,2]
     [1,] "1"  "2"
     [2,] "a"  "b"
...
2) [ ] What R data type/object is ~rbind(1, "Archipel Gulag")~ (and what
   does "Archipel Gulag" mean?)
   
3) [ ] What is ~c(1,2) == c(1,2)~ and why?
...
4) [ ] What is ~matrix(c(c(1,2)))~ - what is ~matrix(c(1, c(1,2)))~?
...
5) [ ] How do you extract elements from ~qux <- c(10,100,1000)~ that are
...
6) [ ] The output show the Star Wars revenue data for three films:
   [[../img/t5matrix.png]]
...
* TODO WED Review naming vectors and indexing
