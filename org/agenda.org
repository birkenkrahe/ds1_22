...
2) You've bought a new car. The car company sends you a survey. What
...
3) How would you create such a data frame for a survey?
   #+begin_quote
...
4) Which R commands are used to explore data frames?
   #+begin_src R
...
6) How do you get values from a data frame? For example:
   - the first row
   - the third column
...
7) How can you add another row to the data frame?
   #+begin_src R
...
8) How can you name the observations or rows?
   #+begin_src R
     names(new_survey)
     rownames(new_survey)
...
9) How can you subset observations? E.g. for car types from 2023?
   #+begin_src R
...
10) How can you extract the ~Make~ from that subset?
    #+begin_src R
