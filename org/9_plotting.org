...
- Using base R and ggplot2
  ...
  These lectures closely follows the presentation in Davies (2016),
...
- Histograms are useful when you have numeric-continuous variables

- Example 1) You've already seen the histogram for a simple data
  structure, the time series (here next to the line plot):
...
- [ ] What is the /binwidth/ of this histogram? Tip: look at its
  /structure/ to find this out.
  #+begin_src R
    str(hist(Nile))
  #+end_src
...
- [ ] Look at the ~help~ for ~hist~ and change the binwidth to ~30~.
...
- Example 2) Create a histogram of the horsepower data of the 32 cars
  in ~mtcars~
